...
export class TradingAgent {
  ...
  constructor(config: AgentConfig) {
    ...
    this.wallet = new WalletService(config.walletConfig.apiKeyName, config.walletConfig.privateKey);
  ...
  async tradingCycle(): Promise<void> {
    ...
    const opportunities = await this.analyzeOpportunities(topTraders, socialSignals);
  ...
  private async analyzeOpportunities(
    ...
    for (const signal of signals) {
      ...
      const historicalData = await this.data.getHistoricalData(signal.tokenAddress);
  ...
  private async validateAndExecuteTrade(signal: MarketSignal): Promise<boolean> {
    try {
      ...
      const tradeAmount = (balance.total * this.config.tradingConfig.maxPositionSize).toString();
      if (!await this.wallet.validateLiquidity(signal.tokenAddress, tradeAmount)) {
